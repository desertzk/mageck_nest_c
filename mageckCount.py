...
def normalizeCounts(ctable,method='median',returnfactor=False,reversefactor=False,negative_control_gRNAs=None):
  """
  ...
  if method=='control':
    ...
    ctable_nm={k:v for (k,v) in ctable.items() if k in negative_control_gRNAs}
...
def mageckcount_printdict(dict0,args,ofile,ounmappedfile,sgdict,datastat,sep='\t'):
  ...
  # print header
  #ofile.write('sgRNA'+sep+'Gene'+sep+sep.join(slabel))
  print('sgRNA'+sep+'Gene'+sep+sep.join(slabel),file=ofile)
  # print items
  if len(sgdict)==0:
    for (k,v) in dict0.items():
      #ofile.write(k+sep+'None'+sep+sep.join([str(x) for x in v]))
    ...
    for (k,v) in dict0.items():
      if k not in sgdict: # only print those in the genedict
        if ounmappedfile != None:
          #ounmappedfile.write(sep.join([k,k])+sep+sep.join([str(x) for x in v]))
          print(sep.join([k,k])+sep+sep.join([str(x) for x in v]),file=ounmappedfile)
        continue
      sx=sgdict[k]
      #ofile.write(sep.join([sx[0],sx[1]])+sep+sep.join([str(x) for x in v]))
    ...
    for (k,v) in sgdict.items():
      if k not in dict0:
        #ofile.write(sep.join([v[0],v[1]])+sep+sep.join(["0"]*nsample))
...
def mageck_printdict(dict0,args,sgdict,sampledict,sampleids):
  ...
  if cntheader !=None:
    #ofile.write('sgRNA\tGene\t'+'\t'.join(cntheader))
    print('sgRNA\tGene\t'+'\t'.join(cntheader),file=ofile)
  if len(sgdict)==0:
    for (k,v) in dict0.items():
      #ofile.write(k+'\t'+'None'+'\t'+'\t'.join([str(x) for x in v]))
    ...
    for (k,v) in dict0.items():
      if k not in sgdict: # only print those in the genedict
        ...
        continue
      #ofile.write('\t'.join([k,sgdict[k]])+'\t'+'\t'.join([str(x) for x in v]))
