...
class SimCaseSimple:
    ...
    def gene_to_printfield(self,onesided=False,wald_only=True):
        '''
        Convert values to print field
        '''
        nsg=0
        if len(self.nb_count)>0:
            nsg_real=len([i for i in self.eff_estimate if i==1])
            nsg=(self.nb_count.shape[1])
        # to field
        ret=[self.prefix,str(nsg_real)]
...
def gene_fdr_correction(allgenedict, method,negative_control,wald_only=False):
  '''
  perform p value correction
  '''
  if wald_only==True:
    pvaluelabel_list=['beta_pval']
  else:
  ...
  for ii in range(len(pvaluelabel_list)):
    ...
    for (gene,ginst) in allgenedict.items():
      if gene not in negative_control:
        tlist=getattr(ginst,whichp)
        pvaluemat_list+=[tlist]
    #
    import numpy as np
    from mageck_nest.fdr_calculation import pFDR
